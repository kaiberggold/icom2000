...
// boundary, matching icom::core::configureLevels()'s style: this reads
...
public:
    ...
    // Defined below, after this class closes: both hold a File by
    ...
    // A missing file is reported via `fileFound = false` with `ok = true`
    ...
    std::string get(std::string_view section, std::string_view key, std::string defaultValue) const;
    ...
    unsigned getUint(std::string_view section, std::string_view key, unsigned defaultValue) const;

    // Splits on commas, trims whitespace around each entry, drops empty
    // entries. Returns an empty vector if the key is absent.
    std::vector<std::string> getList(std::string_view section, std::string_view key) const;
...
struct File::ParseResult {
    File config;
    bool ok = true;
    std::string error; // set (with a 1-based line number) iff !ok
};

struct File::LoadResult {
    File config;   // always valid to read from, even on a missing file
